###########################Figure 4. Dissection of identifying target susceptibility genes in the lung cancer GWAS loci.###############################
...
LCgwas=fread(sprintf("%s/LCgwas_function_permu.xls",plotdir))
LCgwas1=unique(LCgwas[,c("Locus_ID","Cytoband","rsid","cCRE","eqtl_mygene","eqtlfdr_mygene","coloc_mygene")])
length(unique(LCgwas1$Cytoband))
length(unique(LCgwas1$rsid))

# clock
LCgwas1=LCgwas %>%
  select(Locus_ID,Cytoband,rsid,cCRE,eqtl_mygene,eqtlfdr_mygene,coloc_mygene) %>%
  distinct() %>%
  mutate(
    coloc= case_when(coloc_mygene %in% "" ~ 0 ,TRUE ~1),
    eqtlfdr= case_when(eqtlfdr_mygene %in% "" ~ 0 ,TRUE ~1),  
    eqtl= case_when(eqtl_mygene %in% "" ~ 0 ,TRUE ~1),  
    cCRE= case_when(cCRE %in% "" ~ 0 ,TRUE ~1),
    colocpeaks=coloc) %>%
  select(Locus_ID,Cytoband,rsid,colocpeaks,coloc,eqtlfdr,eqtl,cCRE) %>%
  distinct()
LCgwas1=LCgwas1 %>%
...
df <- table(LCgwas1$gourp)
ratio <- prop.table(df) * 100
label <- c("Level 1 & 2", "Level 3","Level 4", "Level 5")
df <- data.frame(ratio, label)

pdf(file=sprintf("%s/pieplot_permu.pdf",plotdir),width=7,heigh=6)
ggplot()+
  ...
  xlab("")+ylab('')+
  scale_fill_manual(values = c('#D293B8', '#E4E4EF', '#DEF5E4', '#79D5AE'))+
  geom_arc_bar(data=df,stat = "pie",aes(x0=0,y0=0,r0=1,r=2,amount=ratio,fill=label))
dev.off()

# Figure 4B Heatmap shows the PP4 of co-localization for target susceptibility genes in the lung cancer GWAS loci that colocalized with GWAS associations for NSCLC.#######################
# get gene order 
lctype=c("LC","AD",'SC')
